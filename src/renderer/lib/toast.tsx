...
export function showToast(message: string, type: "success" | "error" | "info" | "warning") {
  toast.custom(() => (
    <div className={clsx("w-96 flex items-center shadow-md  bg-[#fefefe] p-3 px-4 rounded-md", type === "success" && "shadow-green-400", type === "error" && "shadow-red-400", type === "warning" && "shadow-yellow-400", type === "info" && "shadow-blue-400")}>
      ...
      <h1 className={clsx("font-bold", type === "success" && "text-green-700", type === "error" && "text-red-500", type === "warning" && "text-yellow-500", type === "info" && "text-blue-500")}>{message}</h1>
    </div>
  ));
}
